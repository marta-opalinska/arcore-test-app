...
import com.example.testactivity_arcoreissue.PermissionsHelper
...
class ARCoreSessionLifecycleHelper(
    ...
    // Creating a Session may fail. In this case, sessionCache will remain null, and this function
    ...
    override fun onResume(owner: LifecycleOwner) {
        if (!PermissionsHelper.hasCameraPermission(activity)) {
            PermissionsHelper.requestCameraPermission(activity)
            return
        }
    ...
    fun onRequestPermissionsResult() {
        if (!PermissionsHelper.hasCameraPermission(activity)) {
            Toast.makeText(
                activity,
                "Camera & Storage permission is needed to run this application",
                Toast.LENGTH_LONG
            ).show()
            if (!PermissionsHelper.shouldShowRequestPermissionRationale(activity)) {
                // Permission denied with checking "Do not ask again".
                PermissionsHelper.launchPermissionSettings(activity)
            }
            activity.finish()
        }
    }
